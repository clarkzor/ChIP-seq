...
for rep in "${REPLICATES[@]}"; do
  read1="$RAW_DIR/${SAMPLE_PREFIX}_Rep${rep}.fastq.gz"
  prefix="$OUTPUT_DIR/${SAMPLE_PREFIX}_Rep${rep}"

  require_file "$read1"

  bowtie2 -x "$INDEX_PREFIX" -U "$read1"     -p "${SLURM_CPUS_PER_TASK:-16}"     -S "${prefix}.sam"

  samtools view -@ "${SLURM_CPUS_PER_TASK:-16}" -b     "${prefix}.sam" -o "${prefix}.bam"

  # Historical workflows used samtools rmdup. This template leaves duplicate
  # handling as a distinct documented step rather than silently assuming a method.
